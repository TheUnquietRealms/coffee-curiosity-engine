...
function loadState() {
  try {
    ...
    return { articles: articles.map((a) => ({ ...defaultArticle, ...a, mode: a.mode || 'Essay' })), codex: { ...defaultCodex, ...(parsed.codex || {}) } };
  } catch {
...
export default function App() {
  ...
  function createArticle() {
    ...
    const article = { ...defaultArticle, id, title: 'Untitled Draft', subtitle: '', body: '', mode: 'Essay', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement('button', { onClick: createArticle }, 'New Article'),
      React.createElement('div', { className: 'article-list' }, articles.map((article) => React.createElement(
        'div',
        { key: article.id, className: article.id === selected.id ? 'article-item active' : 'article-item' },
        React.createElement('button', { className: 'article-button', onClick: () => { setSelectedId(article.id); setReview(null); } }, article.title || 'Untitled Draft'),
        React.createElement('span', { className: 'mode-pill' }, article.mode || 'Essay'),
        articles.length > 1 ? React.createElement('button', { className: 'delete-button', onClick: () => deleteArticle(article.id) }, 'Delete') : null
      )))
    ),
    React.createElement(
      'section',
      ...
      React.createElement('div', { className: 'editor-toolbar' },
        React.createElement('select', { value: selected.status, onChange: (event) => updateArticle({ status: event.target.value }) }, ['draft', 'review', 'published'].map((status) => React.createElement('option', { key: status, value: status }, status))),
        React.createElement('select', { value: selected.mode, onChange: (event) => { updateArticle({ mode: event.target.value }); setReview(null); } }, modes.map((mode) => React.createElement('option', { key: mode, value: mode }, mode))),
        React.createElement('button', { onClick: () => exportMarkdown(selected) }, 'Export MD')
      ),
      React.createElement('input', { value: selected.title, onChange: (event) => updateArticle({ title: event.target.value }), placeholder: 'Title' }),
      React.createElement('input', { value: selected.subtitle, onChange: (event) => updateArticle({ subtitle: event.target.value }), placeholder: 'Subtitle' }),
      React.createElement('textarea', { value: selected.body, onChange: (event) => updateArticle({ body: event.target.value }), rows: 18, placeholder: 'Start writing...' }),
      React.createElement('p', { className: 'editor-meta' }, `${words} words · ${readingTime} min read · saved locally`)
    ),
    React.createElement(
      'section',
      { className: 'panel right' },
      React.createElement('h2', null, 'Codex'),
      React.createElement('label', null, 'Voice rules'),
      React.createElement('textarea', { value: codex.voiceRules, onChange: (event) => setCodex({ ...codex, voiceRules: event.target.value }), rows: 3 }),
      React.createElement('label', null, 'Banned AI habits'),
      React.createElement('textarea', { value: codex.bannedHabits, onChange: (event) => setCodex({ ...codex, bannedHabits: event.target.value }), rows: 3 }),
      React.createElement('label', null, 'Recurring themes'),
      React.createElement('textarea', { value: codex.themes, onChange: (event) => setCodex({ ...codex, themes: event.target.value }), rows: 3 }),
      React.createElement('h2', null, 'Editorial Review'),
      React.createElement('span', { className: 'review-mode-badge' }, selected.mode),
      React.createElement('button', { onClick: () => setReview(runReview(selected, codex)) }, 'Run Review'),
      review ? React.createElement('div', null,
        React.createElement('div', { className: 'score' }, String(review.overall)),
        React.createElement('p', null, review.summary),
        review.categories.map((item) => React.createElement('div', { key: item.category, className: 'review-row' },
          React.createElement('strong', null, `${item.category}: ${item.score}`),
          React.createElement('p', null, item.note)
        ))
      ) : React.createElement('p', null, 'Run review when the draft is ready for an editorial pass.')
    )
  );
}
